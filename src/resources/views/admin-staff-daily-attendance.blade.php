...
{{-- 修正: arrow_left -> arrow-left, navigation_arrow -> navigation-arrow --}}
            <a href="?date={{ $currentDate->copy()->subDay()->format('Y-m-d') }}" class="arrow-left"><span class="nav-arrow">&#x2B05;</span>前日</a>
        </div>
...
{{-- 制限を外し、常に翌日への移動を許可します --}}
            <a href="?date={{ $currentDate->copy()->addDay()->format('Y-m-d') }}" class="arrow-right">翌日<span class="nav-arrow">&#x27A1;</span></a>
        </div>
        </div>
    </div>
