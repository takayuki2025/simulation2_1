...
　一般ユーザーを4人作成したので日勤専用(定時9時〜18時),夜勤専用(定時21時〜6時),<br>
　２交代制(日勤,夜勤を1週間ごと)として分けて,<br>
　採点時,修正申請等を入れやすいように平日7％の確率でユーザーごとに出勤なし,<br>
　休憩一般的な1日3回,合計55分〜60分(残業時4回,合計65分〜70分),(勤怠打刻での休憩は何回でもできます)<br>
　残業あり30%の確率としてシーダーファイルを作成致しました。)<br><br>

- MailHogのメール認証機能でその前の新規登録後のページで’認証はこちらから’を押した後,HTMLで’Verify Email Address’ボタン（認証完了ボタン）が表示されない時があるのですが,MailHogのレンダリング失敗らしく,inboxを押して戻り何度か開き直すことで表示されたり,表示されなかったら開くとHTMLタグの右隣にPlain textがあるのでそれを押すと文章の中に’Verify Email Address:’というのがありその右側にURLがあるのでそれをクリックするとメール認証が完了してページに移動します。<br><br>

- 専属コーチに承認を得ましたことで,テーブル内のカラムごとのスペースが少しfigmaと違いますが,<br>
文字数(名前・メール・備考)が多い場合の対策を優先を致しまして,<br>
　管理者専用ページの日ごとの勤怠一覧,スタッフ一覧の名前の文字数は1行10文字まで(申請一覧では名前7文字まで),メールアドレス(ドメイン含む)は1行40文字まで,<br>
　申請一覧(一般ユーザー専用ページも同じ)の備考は8文字までは全て1行で表示できるようにして,名前・備考はそれ以降の文字数の場合は... と表示をして,<br>
...
(管理者)は承認待ち以外の時は何度でも修正できます。<br>
　(一般ユーザー用勤怠詳細ページで修正申請後,承認待ち：＊承認待ちのため修正はできません。(表示専用,休憩しなかった日は'休憩なし'と表示)<br>
　/承認済み：＊この日は一度承認されたので修正できません。(表示専用,休憩しなかった日は'休憩なし'と表示)<br>
　/修正申請なし：修正ボタン設置。(入力専用),のようにメッセージ内容や送信処理ボタンの設置(フォーム等も)を状態によって分けています。)<br><br>

- (一般ユーザー)の勤怠打刻の退勤処理をした後のヘッダーは,figmaと同じく,’今月の出勤一覧’,’申請一覧’,’ログアウト’になります。　<br><br>
...
- Viewファイル<br>
　今回からviewファイル関係はケバブケースで開発するようにしました。模擬案件1つ目はコード品質,Ui要件,マイリストでの検索機能の未実装,新規登録・ログイン時,formrequestでの未実装,ER図の不備等ありましたが,日々良い開発を目指し,今後良い仕事をしながら向上できるように今回からPrettier等を利用して開発効率と可読性,技術を向上させていければと思っています。
- バリデーション関係<br>
...
# 今後開発品質の高い効率の良いWEB開発をしていく上でのまとめ（模擬案件の時だけ掲載）<br>
   ...
   - もくもく会でお話を聞くことができたのですがpro生の,姫野太雅さん,亀山裕樹さん(Java),一倉さやかさん(Ruby),ozaponさんなどのお話で,Laravel以外の案件の開発も携わっているということを聞き,Laravel,他の言語(JavaやRubyもしくはPython)などの案件に私も携わることができたらと,JavaScript(Nuxt.js,React等)を学習しながらですが,模擬案件2つ目終了した後に私も未来を見据えて仕事で良く活かせるように進めてみようと思っています。それから今回はLaravelやPHPなどの最新バージョンアップでの実装も経験しました。(Laravel等のバージョンアップも少し大変でしたがmysqlは特にセキュリティー管理関係で大変でした)。
<br>
